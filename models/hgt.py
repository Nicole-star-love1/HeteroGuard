# -*- coding: utf-8 -*-
"""
HGT backbone for heterogeneous node classification.
...
class HGT(nn.Module):
    ...
    This is the recommended stronger heterogeneous backbone for Freebase-like
    ...
    def _check_metadata(self, data):
        ...
        if self.convs is None:
            ...
            raise ValueError(
                "HGT was initialized with one graph metadata but received another. "
    ...
    def encode(self, data) -> Dict[str, torch.Tensor]:
        ...
        for conv, norm_dict in zip(self.convs, self.norms):
            ...
            for node_type in x_dict:
                h_new = out_dict[node_type] if node_type in out_dict else torch.zeros_like(x_dict[node_type])
                h = x_dict[node_type] + F.dropout(h_new, p=self.dropout, training=self.training)
